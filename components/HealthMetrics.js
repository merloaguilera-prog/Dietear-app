...
export default function HealthMetrics({profile,setProfile,history,register}){
  ...
  const needs=useMemo(()=>{const w=Number(m.weight||profile.weight);if(!w)return null;const kcal=calc.kcal||null;return {
    protein:calc.protein,
    fiber:kcal?Math.round(kcal/1000*14):null,
    carbs:kcal?[Math.round(kcal*.45/4),Math.round(kcal*.60/4)]:null,
    fats:kcal?[Math.round(kcal*.20/9),Math.round(kcal*.35/9)]:null,
    water:[Math.round(w*30/100)/10,Math.round(w*35/100)/10]
  }},[m.weight,profile.weight,calc]);
  ...
  return <div className="metricStudio">
    ...
    <div className="needsPanel">
      ...
      {showNeeds&&<div className="metricCards">
        ...
        <article><small>VITAMINAS Y MINERALES</small><b>Variedad primero</b><span>Prioriza fruta, verdura, legumbres, cereales integrales, proteínas variadas, frutos secos y alimentos ricos en calcio. No se recomiendan suplementos sin valorar necesidades individuales.</span></article>
      </div>}
      {showNeeds&&<p className="helper">Cálculos orientativos para adultos. Embarazo, lactancia, enfermedad renal, hepática, cardiaca, diabetes, medicación u otras situaciones pueden requerir objetivos diferentes; en esos casos DIETEAR debe adaptar el plan con supervisión profesional.</p>}
    </div>
